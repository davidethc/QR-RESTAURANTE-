                ...
                >
                  ...
                  <div className="flex shrink-0 items-center gap-1.5">
                    <Button
                      ...
                      variant="outline"
                      size="icon"
                      className="size-8 rounded-[6px] min-h-8 min-w-8"
                      onClick={() => onUpdateQuantity(index, item.quantity - 1)}
                      aria-label={`Disminuir cantidad de ${item.product.name}`}
                    >
                      <Minus />
                    </Button>
                    <span className="w-6 text-center text-[15px] font-semibold tabular-nums font-mono" aria-live="polite">
                      {item.quantity}
                    </span>
                    <Button
                      type="button"
            ...
            <div className="no-scrollbar -mx-4 flex gap-2 overflow-x-auto px-4 pb-1">
              {suggestionsToShow.map((product) => (
                <button
                  ...
                  className="flex min-h-16 w-[132px] shrink-0 flex-col justify-between gap-1.5 rounded-2xl bg-card px-3 py-2.5 text-left active:bg-muted"
                >
        ...
        {items.length > 0 && (
          <SheetFooter className="gap-3 border-t pt-3">
            ...
            {inTable ? (
              ...
              <p className="rounded-2xl bg-secondary px-3 py-2.5 text-center text-[13px] text-muted-foreground">
